...
with the JSON schema expected by [Energy SOAR](https://energysoar.com).
...
Each flavor's name appear in Energy SOAR's analyzer list and in MISP when you
...
correspond a folder named *VirusTotal\_GetReport\_3\_0* for report templates.
 Report templates are used by Energy SOAR to display the analyzer's JSON output
...
Description of the analyzer. Please be concise and clear. The description is
 shown in the Cortex UI, Ebergy SOAR and MISP.

#### dataTypeList
The list of Energy SOAR datatypes supported by the analyzer. Currently Energy
 SOAR accepts the following datatypes:
...
-   `summary` is used in Energy SOAR for short reports displayed in the
...
### Report Templates
When using Energy SOAR, analysts can submit an observable for analysis to one or
several Cortex instances by a click of a button. Once finished,
Cortex returns the result to Energy SOAR. The Energy SOAR displays that result
using HTML templates for short and long reports.

#### Cortex Result in Energy SOAR
Energy SOAR receives the Cortex result which is simply the JSON formatted
...
named `content` within Energy SOAR.
-   The `full` section is read to display long reports when clicking the short
    ...
    page. In Energy SOAR application, it is stored in a **dict** object named
    `content`.

#### Displayed Information

##### When No Template is Imported
In the event that the analyzer report templates are not imported in Energy SOAR
(only administrators can do such an operation via the *Admin > Report
Templates* menu):

-   In the observable list, Energy SOAR is able to display the analyzer `summary`
...
If templates are imported into Energy SOAR:

-   Short reports are displayed in the observable list and in the detailed
observable page.

![VT short report](../media/sc-short-vt.png)

-   Long reports are displayed when clicking on the short reports or in the
detailed observable page.

![VT long report](../media/sc-long-vt.jpg)


#### Writing Templates

To display results nicely in Energy SOAR, write two HTML templates:

- One for short reports
- One for long reports

When Energy SOAR users import them in the application, they will be definitely
...
Energy SOAR uses Bootstrap and AngularJS so you can leverage them in your
templates.
