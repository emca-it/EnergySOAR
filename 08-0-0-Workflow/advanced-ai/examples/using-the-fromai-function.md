...
When configuring [tools](/08-0-0-Workflow/glossary.rst#ai-tool) connected to the Tools Agent, many parameters can be filled in by the AI model itself. The AI model will use the context from the task and information from other connected tools to fill in the appropriate details.
...
![image showing stars icon to the right of parameter field](/08-0-0-Workflow/_images/advanced-ai/ai-stars.png)

On activating this button, the [AI Agent](/08-0-0-Workflow/glossary.rst#ai-agent) will fill in the expression for you, with no need for any further user input.
...
The `$fromAI()` function uses AI to dynamically fill in parameters for tools connected to the [Tools AI agent](/08-0-0-Workflow/integrations/builtin/cluster-nodes/root-nodes/n8n-nodes-langchain.agent/tools-agent.md).

```{note}
Only for tools

The `$fromAI()` function is only available for tools connected to the AI Agent node. The `$fromAI()` function doesn't work with the [Code](/08-0-0-Workflow/integrations/builtin/cluster-nodes/sub-nodes/n8n-nodes-langchain.toolcode.md) tool or with [other non-tool cluster sub-nodes](/08-0-0-Workflow/integrations/builtin/cluster-nodes/sub-nodes/index.md).
```
...
You can see the `$fromAI()` function in action in the following [templates](/08-0-0-Workflow/glossary.rst#template-n8n):
