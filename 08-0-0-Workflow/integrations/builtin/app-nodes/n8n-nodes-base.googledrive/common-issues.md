...
Here are some common errors and issues with the [Google Drive node](/08-0-0-Workflow/integrations/builtin/app-nodes/n8n-nodes-base.googledrive/index.md) and steps to resolve or troubleshoot them.
...
If you are self-hostin n8n, check the n8n configuration items used to construct external URLs. Verify that the [`N8N_EDITOR_BASE_URL`](/08-0-0-Workflow/hosting/configuration/environment-variables/deployment.md) and [`WEBHOOK_URL`](/08-0-0-Workflow/hosting/configuration/configuration-examples/webhook-url.md) environment variables use fully qualified domains.
