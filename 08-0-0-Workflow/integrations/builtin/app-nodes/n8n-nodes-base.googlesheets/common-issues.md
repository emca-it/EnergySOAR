...
Here are some common errors and issues with the [Google Sheets node](/08-0-0-Workflow/integrations/builtin/app-nodes/n8n-nodes-base.googlesheets/index.md) and steps to resolve or troubleshoot them.
...
1. The [Split Out](/08-0-0-Workflow/integrations/builtin/core-nodes/n8n-nodes-base.splitout.md) node.
1. The [AI Transform](/08-0-0-Workflow/integrations/builtin/core-nodes/n8n-nodes-base.aitransform.md) node. For example, try entering something like:
    ```
    Convert 'languages' array to JSON (key, value) pairs.
    ```
1. The [Code node](/08-0-0-Workflow/integrations/builtin/core-nodes/n8n-nodes-base.code/index.md).
