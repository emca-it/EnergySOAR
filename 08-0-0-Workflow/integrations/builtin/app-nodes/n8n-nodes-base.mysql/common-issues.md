...
Here are some common errors and issues with the [MySQL node](/08-0-0-Workflow/integrations/builtin/app-nodes/n8n-nodes-base.mysql/index.md) and steps to resolve or troubleshoot them.
...
When configuring [MySQL credentials](/08-0-0-Workflow/integrations/builtin/credentials/mysql.md), the `localhost` address should work without a problem (set the **Host** to `localhost`).
...
When configuring [MySQL credentials](/08-0-0-Workflow/integrations/builtin/credentials/mysql.md), use `host.docker.internal` as the **Host** address instead of `localhost`.
...
When configuring [MySQL credentials](/08-0-0-Workflow/integrations/builtin/credentials/mysql.md), use the MySQL container's name as the host address instead of `localhost`. For example, if you call the MySQL container `my-mysql`, you would set the **Host** to `my-mysql`.

### If MySQL and n8n are running in the same Docker container

If MySQL and n8n are running in the same Docker container, the `localhost` address doesn't need any special configuration. You can configure MySQL to listen on `localhost` and configure the **Host** in the [MySQL credentials in n8n](/08-0-0-Workflow/integrations/builtin/credentials/ollama.md) to use `localhost`.
...
As an alternative, you can manually	convert from the string to a decimal using the [`toFloat()` function](/08-0-0-Workflow/code/builtin/data-transformation-functions/strings.md#string-toFloat) with [`toFixed()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Number/toFixed) or with the [Edit Fields (Set) node](/08-0-0-Workflow/integrations/builtin/core-nodes/n8n-nodes-base.set.md) after the MySQL node. Be aware that you may still need to account for a potential loss of precision.
