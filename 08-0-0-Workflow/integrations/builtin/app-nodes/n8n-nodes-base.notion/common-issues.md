...
Here are some common errors and issues with the [Notion node](/08-0-0-Workflow/integrations/builtin/app-nodes/n8n-nodes-base.notion/index.md) and steps to resolve or troubleshoot them.
...
If you need to work with Notion databases with one-way relationship, you can use the [HTTP Request](/08-0-0-Workflow/integrations/builtin/core-nodes/n8n-nodes-base.httprequest/index.md) with your existing Notion credentials. For example, to update a one-way relationship, you can send a `PATCH` request to the following URL:
...
4. Add an [HTTP Request](/08-0-0-Workflow/integrations/builtin/core-nodes/n8n-nodes-base.httprequest/index.md) node connected to the Notion node and select the `GET` method.
...
7. Add an [Edit Fields (Set)](/08-0-0-Workflow/integrations/builtin/core-nodes/n8n-nodes-base.set.md) node after the HTTP Request node.
...
To check for the data before executing the Notion node, use an [If](/08-0-0-Workflow/integrations/builtin/core-nodes/n8n-nodes-base.if.md) node to check whether the field is unset. This allows you to use the [Edit Fields (Set)](/08-0-0-Workflow/integrations/builtin/core-nodes/n8n-nodes-base.set.md) node to conditionally remove the field when it doesn't have a valid value.

As an alternative, you can set a [default value](/08-0-0-Workflow/code/cookbook/expressions/check-incoming-data.md) if the incoming data doesn't provide one.
