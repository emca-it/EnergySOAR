...
/// note
Refer to [Oracle Database credentials](/08-0-0-Workflow/integrations/builtin/credentials/oracledb.md) for guidance on setting up authentication.
...
- **Credential to connect with**: Create or select an existing [Oracle Database credential](/08-0-0-Workflow/integrations/builtin/credentials/oracledb.md).
...
- **Statement**: The SQL statement to execute. You can use n8n [expressions](/08-0-0-Workflow/code/expressions.md) and positional parameters like `:1`, `:2`, or named parameters like `:name`, `:id` to use with [Use bind parameters](#use-bind-parameters).
...
- **Mapping Column Mode**: How to map column names to incoming data:
	...
	- **Map Automatically**: Automatically map incoming data to matching column names in Oracle Database. The incoming data field names must match the column names in Oracle Database for this to work. If necessary, consider using the [edit fields (set) node](/08-0-0-Workflow/integrations/builtin/core-nodes/n8n-nodes-base.set.md) before this node to adjust the format as needed.
