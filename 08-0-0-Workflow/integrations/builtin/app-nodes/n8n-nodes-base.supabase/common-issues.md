...
Here are some common errors and issues with the [Supabase node](/08-0-0-Workflow/integrations/builtin/app-nodes/n8n-nodes-base.supabase/index.md) and steps to resolve or troubleshoot them.
...
When configuring [Supabase credentials](/08-0-0-Workflow/integrations/builtin/credentials/supabase.md), the `localhost` address should work without a problem (set the **Host** to `localhost`).
