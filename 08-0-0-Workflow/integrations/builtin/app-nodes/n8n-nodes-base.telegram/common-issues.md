...
Here are some common errors and issues with the [Telegram node](/08-0-0-Workflow/integrations/builtin/app-nodes/n8n-nodes-base.telegram/index.md) and steps to resolve or troubleshoot them.
...
1. From the Telegram Trigger: Use the [Telegram Trigger](/08-0-0-Workflow/integrations/builtin/trigger-nodes/n8n-nodes-base.telegramtrigger/index.md) node in your workflow to get a Chat ID. This node can trigger on different events and returns a Chat ID on successful execution.
...
1. **Loop Over Items node**: Use the [Loop Over Items](/08-0-0-Workflow/integrations/builtin/core-nodes/n8n-nodes-base.splitinbatches.md) node to get at most 30 chat IDs from your database.
...
3. **Code node**: Connect the [Code](/08-0-0-Workflow/integrations/builtin/core-nodes/n8n-nodes-base.code/index.md) node with the Telegram node. Use the Code node to wait for a few seconds before fetching the next batch of chat IDs. Connect this node with the Loop Over Items node.

You can also use this [workflow](https://n8n.io/workflows/772).

## Remove the n8n attribution from sent messages

If you're using the node to [send Telegram messages](/08-0-0-Workflow/integrations/builtin/app-nodes/n8n-nodes-base.telegram/message-operations.md#send-message), the message automatically gets an n8n attribution appended to the end:
...
Refer to [Send Message additional fields](/08-0-0-Workflow/integrations/builtin/app-nodes/n8n-nodes-base.telegram/message-operations.md#send-message-additional-fields) for more information.
