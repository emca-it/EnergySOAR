...
Here are some common errors and issues with the [Structured Output Parser node](/08-0-0-Workflow/integrations/builtin/cluster-nodes/sub-nodes/n8n-nodes-langchain.outputparserstructured/index.md) and steps to resolve or troubleshoot them.

## Processing parameters

The Structured Output Parser node is a [sub-node](/08-0-0-Workflow/glossary.md#sub-node-n8n). Sub-nodes behave differently than other nodes when processing multiple items using expressions.
...
You can attach output parser nodes to select [AI root nodes](/08-0-0-Workflow/integrations/builtin/cluster-nodes/root-nodes/index.md).
...
Structured output parsing is often not reliable when working with [agents](/08-0-0-Workflow/integrations/builtin/cluster-nodes/root-nodes/n8n-nodes-langchain.agent/index.md).

If your workflow uses agents, n8n recommends using a separate [LLM-chain](/08-0-0-Workflow/integrations/builtin/cluster-nodes/root-nodes/n8n-nodes-langchain.chainllm.md) to receive the data from the agent and parse it. This leads to better, more consistent results than parsing directly in the agent workflow.
