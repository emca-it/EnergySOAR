...
Here are some common errors and issues with the [Code node](/08-0-0-Workflow/integrations/builtin/core-nodes/n8n-nodes-base.code/index.md) and steps to resolve or troubleshoot them.
...
* Read the [data structure](/08-0-0-Workflow/data/data-structure.md) to understand the data you receive in the Code node and the requirements for outputting data from the node.
* Understand how data items work and how to connect data items from previous nodes with [item linking](/08-0-0-Workflow/data/data-mapping/data-item-linking/index.md).
...
To resolve this, ensure that your Code node returns the [expected data structure](/08-0-0-Workflow/data/data-structure.md):
...
n8n doesn't support importing modules in the [Cloud](/08-0-0-Workflow/manage-cloud/overview.md) version.
```

If you're [self-hosting](/08-0-0-Workflow/hosting/index.md) n8n, follow these steps:

* Install the module into your n8n environment.
	* If you are running n8n with [npm](/08-0-0-Workflow/hosting/installation/npm.md), install the module in the same environment as n8n.
	* If you are running n8n with [Docker](/08-0-0-Workflow/hosting/installation/docker.md), you need to extend the official n8n image with a [custom image](https://docs.docker.com/build/building/base-images/) that includes your module.
* Set the `NODE_FUNCTION_ALLOW_BUILTIN` and `NODE_FUNCTION_ALLOW_EXTERNAL` [environment variables](/08-0-0-Workflow/hosting/configuration/configuration-examples/modules-in-code-node.md) to allow importing modules.
...
To set, update, and retrieve data directly to a workflow, use the [static data](/08-0-0-Workflow/code/cookbook/builtin/get-workflow-static-data.md) functions within your code. You can manage data either globally or tied to specific nodes.

```{info} Use Remove Duplicates when possible
If you're interested in using variables to avoid processing the same data items more than once, consider using the [Remove Duplicates node](/08-0-0-Workflow/integrations/builtin/core-nodes/n8n-nodes-base.removeduplicates/index.md) instead. The Remove Duplicates node can save information across executions to avoid processing the same items multiple times.
```
