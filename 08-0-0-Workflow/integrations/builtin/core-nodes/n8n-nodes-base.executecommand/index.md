...
The Execute Command node can introduce significant security risks in environments that operate with untrusted users. Because of this, the node is [disabled](/08-0-0-Workflow/hosting/securing/blocking-nodes.md#exclude-nodes) by default starting from version 2.0.
```
...
If you're using [queue mode](/08-0-0-Workflow/hosting/scaling/queue-mode.md), the command runs on the worker that's executing the task in production mode. When running manual executions, it runs on the main instance, unless you set `OFFLOAD_MANUAL_EXECUTIONS_TO_WORKERS` to `true`.
```
...
You can also use the [HTTP Request](/08-0-0-Workflow/integrations/builtin/core-nodes/n8n-nodes-base.httprequest/index.md) node to make a cURL request.
...
For common questions or issues and suggested solutions, refer to [Common Issues](/08-0-0-Workflow/integrations/builtin/core-nodes/n8n-nodes-base.executecommand/common-issues.md).
