...
```{note} Credentials
You can find authentication information for this node [here](/08-0-0-Workflow/integrations/builtin/credentials/ftp.md).
```

To connect to an SFTP server, use an SFTP credential. Refer to [FTP credentials](/08-0-0-Workflow/integrations/builtin/credentials/ftp.md) for more information.
...
To attach a file for upload, you'll need to use an extra node such as the [Read/Write Files from Disk](/08-0-0-Workflow/integrations/builtin/core-nodes/n8n-nodes-base.readwritefile.md) node or the [HTTP Request](/08-0-0-Workflow/integrations/builtin/core-nodes/n8n-nodes-base.httprequest/index.md) node to pass the file as a data property.
```
