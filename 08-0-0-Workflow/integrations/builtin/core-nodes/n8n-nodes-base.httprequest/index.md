...
The HTTP Request node is one of the most versatile nodes in n8n. It allows you to make HTTP requests to query data from any app or service with a REST API. You can use the HTTP Request node a regular node or attached to an [AI agent](/08-0-0-Workflow/integrations/builtin/cluster-nodes/root-nodes/n8n-nodes-langchain.agent/tools-agent.md) to use as a [tool](/08-0-0-Workflow/advanced-ai/examples/understand-tools.md){ data-preview }.
...
```{note} Credentials
Refer to [HTTP Request credentials](/08-0-0-Workflow/integrations/builtin/credentials/httprequest.md) for guidance on setting up authentication. 
```
...
Credentials for integrations supported by n8n, including both built-in and community nodes. Use **Predefined Credential Type** for custom operations without extra setup. Refer to [Custom API operations](/08-0-0-Workflow/integrations/custom-operations.md) for more information.
...
Refer to [HTTP request credentials](/08-0-0-Workflow/integrations/builtin/credentials/httprequest.md) for more information on setting up each credential type.
...
For example setups, refer to [HTTP Request node cookbook | Pagination](/08-0-0-Workflow/code/cookbook/http-node/pagination.md).

n8n provides built-in variables for working with HTTP node requests and responses when using pagination:

```{include} ../../../../../_snippets/integrations/builtin/core-nodes/http/pagination-variables.md

```

```{include} ../../../../../_snippets/integrations/builtin/core-nodes/http/pagination-api-differences.md
...
Enter the **Proxy** the request should use. This takes precedence over global settings defined with the [`HTTP_PROXY`, `HTTPS_PROXY`, or `ALL_PROXY` environment variables](/08-0-0-Workflow/hosting/configuration/environment-variables/deployment.md).
...
The following options are only available when attached to an [AI agent](/08-0-0-Workflow/integrations/builtin/cluster-nodes/root-nodes/n8n-nodes-langchain.agent/tools-agent.md) as a [tool](/08-0-0-Workflow/advanced-ai/examples/understand-tools.md){ data-preview }.
...
For common questions or issues and suggested solutions, refer to [Common Issues](/08-0-0-Workflow/integrations/builtin/core-nodes/n8n-nodes-base.httprequest/common-issues.md).
