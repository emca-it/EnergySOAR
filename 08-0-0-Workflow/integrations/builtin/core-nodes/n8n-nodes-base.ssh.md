...
```{note} Credentials
You can find authentication information for this node [here](/08-0-0-Workflow/integrations/builtin/credentials/ssh.md).
```
...
To attach a file for upload, you will need to use an extra node such as the [Read/Write Files from Disk](/08-0-0-Workflow/integrations/builtin/core-nodes/n8n-nodes-base.readwritefile.md) node or the [HTTP Request](/08-0-0-Workflow/integrations/builtin/core-nodes/n8n-nodes-base.httprequest/index.md) node to pass the file as a data property.
```
...
- **Credential to connect with**: Select an existing or create a new [SSH credential](/08-0-0-Workflow/integrations/builtin/credentials/ssh.md) to connect with.
