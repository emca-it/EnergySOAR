...
```{note} Credentials
Refer to [TOTP credentials](/08-0-0-Workflow/integrations/builtin/credentials/totp.md) for guidance on setting up authentication. 
```
...
Select or create a [TOTP credential](/08-0-0-Workflow/integrations/builtin/credentials/totp.md) for the node to use.
