...
n8n provides different **Webhook URL**s for testing and production. The testing URL includes an option to **Listen for test event**. Refer to [Workflow development](/08-0-0-Workflow/integrations/builtin/core-nodes/n8n-nodes-base.webhook/workflow-development.md) for more information on building, testing, and shifting your Webhook node to production.
...
![Sample Webhook URLs in the Webhook node's Parameters tab display a Test URL and Production URL](/08-0-0-Workflow/_images/integrations/builtin/core-nodes/webhook/webhook-urls.png)
...
* PUT
  ...
  If you're self-hosting n8n, you can change this using the [endpoint environment variable](/08-0-0-Workflow/hosting/configuration/environment-variables/endpoints.md) `N8N_PAYLOAD_SIZE_MAX`.
	///	
...
Refer to [Webhook credentials](/08-0-0-Workflow/integrations/builtin/credentials/webhook.md) for more information on setting up each credential type.
...
* **Using 'Respond to Webhook' Node**: The Webhook node responds as defined in the [Respond to Webhook](/08-0-0-Workflow/integrations/builtin/core-nodes/n8n-nodes-base.respondtowebhook.md) node.
* **Streaming response**: Enables real-time data streaming back to the user as the workflow processes. Requires nodes with streaming support in the workflow (for example, the [AI agent](/08-0-0-Workflow/integrations/builtin/cluster-nodes/root-nodes/n8n-nodes-langchain.agent/index.md) node).
...
For common questions or issues and suggested solutions, refer to [Common issues](/08-0-0-Workflow/integrations/builtin/core-nodes/n8n-nodes-base.webhook/common-issues.md).
