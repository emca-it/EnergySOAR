...
Here are some common errors and issues with the [Telegram Trigger node](/08-0-0-Workflow/integrations/builtin/trigger-nodes/n8n-nodes-base.telegramtrigger/index.md) and steps to resolve or troubleshoot them.
...
When running n8n behind a reverse proxy, you must [configure the `WEBHOOK_URL` environment variable](/08-0-0-Workflow/hosting/configuration/configuration-examples/webhook-url.md) with the public URL where your n8n instance is running. For Telegram, this URL must use HTTPS.
