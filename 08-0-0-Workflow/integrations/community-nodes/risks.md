...
In addition to publicly available community nodes from npm, n8n inspects some nodes and makes them available as [verified community node inside the nodes panel](/08-0-0-Workflow/integrations/community-nodes/installation/verified-install.md). These nodes have to meet a set of data and system security requirements for approval.
```
...
If you are self-hosting n8n, you can disable community nodes by setting `N8N_COMMUNITY_PACKAGES_ENABLED` to `false`. On n8n cloud, visit the [Cloud Admin Panel](/08-0-0-Workflow/manage-cloud/cloud-admin-dashboard.md) and disable community nodes from there. See [troubleshooting](/08-0-0-Workflow/integrations/community-nodes/troubleshooting.md) for more information.
