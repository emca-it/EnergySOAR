...
All node's use n8n's [node UI elements](/08-0-0-Workflow/integrations/creating-nodes/build/reference/ui-elements.md), so you don't need to consider style details such as colors, borders, and so on. However, it's still useful to go through a basic design process:
...
* Info boxes: yellow boxes that appear between fields. Refer to [UI elements | Notice](/08-0-0-Workflow/integrations/creating-nodes/build/reference/ui-elements.md#notice) for more information.
...
* Node hints: provide help in the input panel, output panel, or node details view. Refer to [UI elements | Hints](/08-0-0-Workflow/integrations/creating-nodes/build/reference/ui-elements.md#hints) for more information.
* Tooltips: callouts that appear when the user hovers over the tooltip icon !["Screenshot of the tooltip icon. The icon is a ? in a grey circle"](/08-0-0-Workflow/_images/common-icons/help-tooltip.png). Use tooltips for extra information that the user might need.
...
Refer to [Node Error Handling](/08-0-0-Workflow/integrations/creating-nodes/build/reference/error-handling.md) for more information.
...
* Wherever possible, provide two ways to specify a record:
    * By choosing from a pre-populated list. You can generate this list using the `loadOptions` parameter. Refer to [Base files](/08-0-0-Workflow/integrations/creating-nodes/build/reference/node-base-files/index.md) for more information.
    * By entering an ID.
* Name the field `<Record name> name or ID`. For example, **Workspace Name or ID**. Add a tooltip saying "Choose a name from the list, or specify an ID using an expression." Link to n8n's [Expressions](/08-0-0-Workflow/code/expressions.md) documentation.
...
```{include} ../../../../_snippets/integrations/node-icons.md
```
